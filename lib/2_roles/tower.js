...
Tower.prototype.requestTask = function (job) {
 if (!Memory.rooms[this.room.name].towers[this.id]) {
  Memory.rooms[this.room.name].towers[this.id] = {};
 }
 Memory.rooms[this.room.name].towers[this.id][job] = Game.rooms[this.room.name].releaseTask('tower', job);
 if (Memory.rooms[this.room.name].towers[this.id][job]) {
  Memory.rooms[this.room.name].tower[job].push(Memory.rooms[this.room.name].towers[this.id][job]);
  return true;
 } else {
  Memory.rooms[this.room.name].towers[this.id][job] = null;
  return false;
 }
};
