StructureRampart.prototype.report = function () {
 if (this.room.controller.my) {
  ...
  //defensive close
  if (Memory.rooms[this.room.name].tasks.whack.length > 0) {
   for (let e in Memory.rooms[this.room.name].tasks.whack) {
    let enemy = Game.getObjectById(Memory.rooms[this.room.name].tasks.whack[e]);
